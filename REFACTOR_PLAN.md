...
- [x] Improve readability of dense functions: `drinkNow()`/`yDrinkNow()`/`drinkTogether()` DRY'd into shared `executeDrink()` helper with `companionRefusesDrink()`, `generateDrinkQuotes()`, `applyDrinkStats()`. Remaining candidates: `selectitem()`, `champagneNow()`, `giveHer()`, `buyItem()`, `getAmountOwned()`, and dense functions in `bladder.ts`, `quotes.ts`
- [x] Clean up dead code: removed unused `pickRandom()`, `randomIndex()`, `shuffle()`, `gameSexYou()` from main.ts. Fixed `wrapAndFormatAll` pre-existing bug (was called but never defined in darts.ts) — implemented in quotes.ts, imported in darts.ts, removed dead `declare` from globals.d.ts
